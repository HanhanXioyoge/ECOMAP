function [rmse_final, exp_out, sim_out] = abc_max(ecModel, kcat_random_all, growthdata, UnconstrainedGrowth, C13Fluxdata, proc, sample_generation, j, bioRxn, c_source, rxn2block, org_name)
        ...
        if ~isempty(C13Fluxdata) && isstruct(C13Fluxdata) && isfield(C13Fluxdata, 'reactions')
            try
                ...
                for condIdx = 1:nConditions
                    [rmse_c, ~, ~] = rmsecal_13C(ecModel, C13Fluxdata, condIdx, bioRxn, c_source);
