function [relErr,errVar,sumDelta,objVal,avJD,corrKcatProts] = cvLambdaFitting_test(varargin)
...
for l=1:numel(lambdaParams)
    ...
    for i=1:nIter
        ...
        if runParallel
            ...
            parfor s=1:kfold
                ...
                models = BatchModelgeneration(model,pTot(trainIdx),nutrExch(:,trainIdx));
                
                % run kcat correction
                solution.stat = 0;
                tmpModels = models;
                try
                    [solution,tmpModels,~,~,LP] = PRESTOforECOMAP(models,expGrowth(trainIdx),E(:,trainIdx),...
                ...
                if solution.stat == 1
                    ...
                    models = BatchModelgeneration(corrBatchModel,pTot(testIdx),...
                        nutrExch(:,testIdx));
                    ...
                    [~,~,pids] = findKcatCorrections_ECOMAP(model,tmpModels{1},enzMetPfx);
            ...
            for s=1:kfold
                ...
                if solution.stat == 1
                    ...
                    % add pool draw reactions to corrected model
                    corrBatchModel = tmpModels{1};
                    corrBatchModel = updateProtPool(corrBatchModel);
                    ...
                    for j=1:numel(models)
                        ...
                        sol = optimizeCbModel(models{j});
                        fprintf('  Cond %d: sol.f = %.4f, exp = %.4f\n', j, sol.f, expGrowth(testIdx(j)));
                        tmpErr(j) = abs(sol.f-expGrowth(testIdx(j)))/expGrowth(testIdx(j));
                    end
    ...
    function p = parseInput(arguments)
        ...
        validateModel = @(m)isstruct(m)&isfield(m,'enzymeConstraints');
