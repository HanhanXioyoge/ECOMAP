function [model, epsilon, theta, runParallel]= getPRESTOPreparation(model, parameters)
    ...
    if ~isfield(model,'enzymeConstraints')
        error('model.enzymeConstraints is required.');
    end

    epsilon     = parameters.PRESTO.epsilon;
    theta       = parameters.PRESTO.theta;
    runParallel = parameters.PRESTO.runParallel;
    ncpu        = parameters.PRESTO.ncpu;
    %{
    comptype    = parameters.PRESTO.comptype;
    comps       = parameters.PRESTO.comps;
    bioRxnID    = parameters.bioRxn;
    %}
    % model = buildBiomassPseudoRxns(model, comps, comptype, bioRxnID);
