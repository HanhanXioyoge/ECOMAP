function f = objectiveFcn_2(x, ecModel, fit_data, kcat_list)
    ...
    for i = 1:length(fit_data)
        ...
        error = abs(predict_value - target_value);
        if error <= 0.2
            objective_value_addition = 0.5 * error^2;
        else
            objective_value_addition = 0.2 * (error - 0.1);
        end
